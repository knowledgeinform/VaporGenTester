...
class HumidityClass {
  ...
  process(d) {
    ...
    var tempPV
    if (this.serial) {
      tempPV = Number(d.substring(37, 42)) // 4, 10
    } else {
      tempPV = Number(d.substring(51, 57))
    }
    if (isNaN(tempPV)) {
      return
    }
  ...
  dryMFCmassFlow() {
    if (this.services[1].obj.A['Mass Flow'].value) {
  ...
  wetMFCmassFlow() {
    if (this.services[1].obj.E['Mass Flow'].value) {
      return this.services[1].obj.E['Mass Flow'].value.value
    }
    return 0
  }
